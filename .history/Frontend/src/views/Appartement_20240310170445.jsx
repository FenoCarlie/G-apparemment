...
function Appartement() {
  ...
  const handleUpdate = (event) => {
    ...
    setLoading(true);
    axios
      .put(`http://localhost:6009/api/update/${selectedAppartement._id}`, {
        numApp: appartement.numApp,
        design: appartement.design,
        loyer: appartement.loyer,
      })
      ...
      <div className="pl-6 pt-6 flex justify-between">
                  ...
                  {appartements && appartements.length > 0
                    ? appartements.map((item, index) => (
                        <tr
                          key={index}
                          className="bg-white"
                          onClick={() => handleAppartementClick(item)}
                        >
                          <th
                            scope="row"
                          ...
                          <td className="px-6 py-4">{item.design}</td>
                          <td className="px-6 py-4">{item.loyer}</td>
        ...
        >
          <div className={field ? "" : "hidden"}>
            ...
            <div className="h-2 bg-[#f3f3f3]"></div>
            <form
              onSubmit={type === "create" ? handleSubmit : handleUpdate}
